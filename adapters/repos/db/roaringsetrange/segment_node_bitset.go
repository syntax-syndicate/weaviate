...
func (sn *SegmentNodeBS) Len() uint64 {
	return binary.LittleEndian.Uint64(sn.data[0:8])
}

func (sn *SegmentNodeBS) Key() uint8 {
...
func (sn *SegmentNodeBS) Additions() BitSet {
	sn.rw.MoveBufferToAbsolutePosition(9)
	return BitSetFromBuffer(sn.rw.ReadBytesFromBufferWithUint64LengthIndicator())
}

// Deletions returns the deletions roaring bitmap with shared state. Only use
// this method if you can guarantee that you will only use it while holding a
// maintenance lock or can otherwise be sure that no compaction can occur.
func (sn *SegmentNodeBS) Deletions() BitSet {
	sn.rw.MoveBufferToAbsolutePosition(8)
...
func NewSegmentNodeBS(key uint8, additions, deletions BitSet) (*SegmentNodeBS, error) {
...
// negative effects.
func (sn *SegmentNodeBS) ToBuffer() []byte {
...
func NewSegmentNodeBSFromBuffer(buf []byte) *SegmentNodeBS {
	return &SegmentNodeBS{
		data: buf,
