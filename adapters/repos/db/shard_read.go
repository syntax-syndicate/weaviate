...
func (s *Shard) vectorByIndexID(ctx context.Context, indexID uint64, targetVector string, callerId int) ([]float32, error) {
	keyBuf := make([]byte, 8)
	return s.readVectorByIndexIDIntoSlice(ctx, indexID, &common.VectorSlice{Buff8: keyBuf}, targetVector, callerId)
}

func (s *Shard) readVectorByIndexIDIntoSlice(ctx context.Context, indexID uint64, container *common.VectorSlice, targetVector string, callerId int) ([]float32, error) {
