...
func (flat *flat) getBQVector(ctx context.Context, id uint64, callerId int) ([]uint64, error) {
...
func (index *flat) findTopVectorsCached(heap *priorityqueue.Queue[any],
	...
	for ; id < uint64(all) && (allow == nil || id <= allowMax); id++ {
		if allow == nil || allow.Contains(id) {
			vec, err := index.bqCache.Get(context.Background(), id, 0)
			if err != nil {
				return err
			}
			if len(vec) == 0 {
	...
	case compressionBQ:
		if index.bqCache == nil {
			...
			distFunc = func(nodeID uint64) (float32, error) {
				...
				vec, err := index.bqCache.Get(context.Background(), nodeID, 0)
				if err != nil {
					return 0, err
				}
